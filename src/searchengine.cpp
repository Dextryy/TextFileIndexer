    ...
    const QString& fileMask, const QDate& from, const QDate& to)
{
    ...
    const QString word = caseSensitive ? query : query.toLower(); // готовим слово с учЄтом регистра
    ...
    q.prepare(sql);
    q.bindValue(":word", word); // прив€зка параметра :word
    bindFilters(q, fileMask, from, to); // прив€зка :mask/:from/:to
    if (!q.exec()) { qWarning() << q.lastError(); return out; } // если ошибка Ч отдаЄм пустой список

    while (q.next()) { // идем по результатам
        const QString path = q.value(0).toString(); // путь
        const QString modified = q.value(1).toString(); // дата изменени€ 
        ...
        for (const QString& p : parts) { // дл€ каждого номера строки
            const int lineNo = p.toInt(); // переводим в int
            const QString lineText = readLine(path, lineNo); // читаем конкретную строку
            if (!lineText.isEmpty())
                out.push_back({ path, lineNo, lineText, modified, size }); // добавл€ем результат
        }
    }
    return out;
}

// поиск по регул€рному выражению
    ...
    const QString& fileMask, const QDate& from, const QDate& to)
{
    ...
    if (!db || pattern.isEmpty()) return out; // без шаблона Ч нет поиска

    QString sql = "SELECT path, modified, size FROM Files WHERE 1=1 "; // берЄм список файлов
    appendFilters(sql, "Files", fileMask, from, to); // ограничиваем по маске/датам

    QSqlQuery q(db->database()); // запрос 
    q.prepare(sql);
    bindFilters(q, fileMask, from, to); // прив€зываем параметры
    if (!q.exec()) { qWarning() << q.lastError(); return out; } // ошибка Ч пусто
    ...
    while (q.next()) {
        ...
        while (!in.atEnd()) { // читаем построчно
            ...
            if (re.match(line).hasMatch()) // если паттерн нашЄл совпадение
                out.push_back({ path, lineNo, line, modified, size }); // добавл€ем результат
        }
    }
    return out;
}
